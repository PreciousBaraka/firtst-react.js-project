...
const PatientDetails = () => {
  ...
  const { userDetails, loading, patientQueries } = useSelector((state) => state.user);
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="p-4 max-w-4xl mx-auto">
        <h1 className="text-2xl font-semibold text-gray-700 mb-4">My Details</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-6">
          <div className="bg-white shadow-md rounded-lg p-4 text-center border border-gray-100">
            ...
            <p className="text-2xl font-bold text-blue-600">{hospitalVisits?.length || 0}</p>
          </div>

          <div className="bg-white shadow-md rounded-lg p-4 text-center border border-gray-100">
            <h3 className="text-lg font-semibold text-gray-600">Total Treatments</h3>
            <p className="text-2xl font-bold text-green-600">{totalTreatments}</p>
          </div>
        </div>
        ...
        {activeTab === "profile" && (
          <div className="bg-white shadow-md rounded-lg p-4">
            ...
            <CustomFormItem label="Phone Number" value={user?.phoneNumber} readOnly />
        ...
        {activeTab === "visits" && (
          <div>
            <h2 className="text-lg font-semibold mb-2">Hospital Visits & Treatments</h2>
            {hospitalVisits?.length === 0 ? (
              <p className="text-gray-600">No hospital visits found.</p>
            ) : (
              <ul className="space-y-4">
                {hospitalVisits.map((visit) => (
                  <li key={visit.id} className="bg-gray-50 p-4 rounded-lg shadow-sm">
                    <p className="font-medium text-gray-800">Reason: {visit.reason}</p>
        ...
        {activeTab === "queries" && (
          <div className="bg-white shadow-md rounded-lg p-4">
            <h2 className="text-lg font-semibold mb-4">My Queries & Doctor Responses</h2>
            {!patientQueries || patientQueries.length === 0 ? (
              <p className="text-gray-600">No queries submitted.</p>
            ) : (
              <ul className="space-y-4">
                {patientQueries.map((query) => (
                  <li key={query.id} className="border border-gray-200 p-4 rounded">
                    <h3 className="font-semibold text-gray-700">{query.title}</h3>
                    <p className="text-sm text-gray-600 mb-2">{query.description}</p>

                    {query.doctorResponse ? (
                      <div className="bg-green-50 p-2 rounded border border-green-200">
                        <p className="text-sm text-green-700 font-medium">Doctor's Response:</p>
                        <p className="text-sm text-gray-700">{query.doctorResponse.response}</p>
                      </div>
                    ) : (
                      <p className="text-sm text-yellow-600 italic">Awaiting doctor's response...</p>
                    )}
                  </li>
                ))}
              </ul>
